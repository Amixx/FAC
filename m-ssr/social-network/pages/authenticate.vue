        ...
        <div class="md:space-y-6 p-6 sm:p-8 space-y-4">
          ...
          <form class="md:space-y-6 space-y-4" @submit.prevent="authenticate">
            <div>
              ...
              <input
                id="email"
                class="bg-gray-50 block border border-gray-300 focus:border-emerald-600 focus:ring-emerald-600 p-2.5 rounded-lg sm:text-sm text-gray-900 w-full"
                name="email"
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-start">
                <div class="flex h-5 items-center">
                  <input
                    id="remember"
                    class="bg-gray-50 border border-gray-300 focus:ring-3 focus:ring-emerald-300 h-4 rounded w-4"
                    type="checkbox"
                  />
                </div>
              ...
              <a
                class="font-medium hover:underline text-emerald-600 text-sm"
                href="#"
                >Aizmirsi paroli?</a
              >
            </div>
            <button
              class="bg-emerald-600 focus:outline-none focus:ring-4 focus:ring-emerald-300 font-medium hover:bg-emerald-700 px-5 py-2.5 rounded-lg text-center text-sm text-white w-full"
              type="submit"
            >
...
const authenticate = async () => {
  try {
    const data = await $fetch<{ user: User; redirect: string }>(
