    ...
    <div class="flex flex-col gap-1">
      <div class="flex gap-2">
        ...
        <router-link
          ...
          :to="{ name: 'NEW_POST', params: { postId: post.id } }"
        >
          Pārpublicēt
        </router-link>
      </div>
...
const toggleLike = async () => {
  try {
    const flash = (await (
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/posts/toggle-like`, {
        method: 'POST',
...
const deletePost = async () => {
  try {
    const flash = (await (
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/posts/delete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: props.post.id,
